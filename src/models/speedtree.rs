...
fn parse_ctable_format(data: &[u8]) -> Result<SpeedTreeMesh, Report<SpeedTreeError>> {
    ...
    if vertex_stride != CTABLE_VERTEX_STRIDE as u32 {
        return Err(Report::new(SpeedTreeError::UnsupportedStride(
            vertex_stride,
        )));
    }
    if vertex_count == 0 {
    ...
    let vertices: Vec<([f32; 3], [f32; 3], [f32; 2])> = repeat(vertex_count, parse_ctable_vertex)
        ...
        .map_err(|e: ErrMode<ContextError>| {
            Report::new(SpeedTreeError::ParseError(format!("vertex parse: {e}")))
        })?;

    // Parse indices (LE u16)
    let ib_input = &mut &data[ib_start..ib_end];
    let raw_indices: Vec<u16> =
        repeat(index_count, le_u16)
            .parse_next(ib_input)
            .map_err(|e: ErrMode<ContextError>| {
                Report::new(SpeedTreeError::ParseError(format!("index parse: {e}")))
            })?;
...
fn parse_stdk_format(data: &[u8]) -> Result<SpeedTreeMesh, Report<SpeedTreeError>> {
    ...
    let _version = le_u32
        .parse_next(input)
        .map_err(|e: ErrMode<ContextError>| {
            Report::new(SpeedTreeError::ParseError(format!("version: {e}")))
        })?;
    let lod_count = le_u32
        .parse_next(input)
        .map_err(|e: ErrMode<ContextError>| {
            Report::new(SpeedTreeError::ParseError(format!("lod_count: {e}")))
        })? as usize;
    let _has_billboard = le_u32
        .parse_next(input)
        .map_err(|e: ErrMode<ContextError>| {
            Report::new(SpeedTreeError::ParseError(format!("billboard: {e}")))
        })?;
    ...
    let dc_count = le_u32
        .parse_next(input)
        .map_err(|e: ErrMode<ContextError>| {
            Report::new(SpeedTreeError::ParseError(format!("dc_count: {e}")))
        })? as usize;

    for _ in 0..dc_count {
        parse_stdk_draw_call(input, &mut positions, &mut normals, &mut uvs, &mut indices).map_err(
            |e: ErrMode<ContextError>| {
                Report::new(SpeedTreeError::ParseError(format!("draw_call: {e}")))
            },
        )?;
    }
