...
pub trait Prime {
    fn prime_volume(&self, volume: &str, range: Range<usize>)
...
fn build_vfs_entries(tree: &BTreeMap<String, VfsEntry>) -> HashMap<String, VfsEntryMeta> {
    ...
    for (path, entry) in tree {
        let meta = match entry {
            ...
            VfsEntry::Directory => VfsEntryMeta::Directory {
                children: Vec::new(),
            },
        };
    ...
    T: Prime + Debug + Send + Sync + 'static,
{
    ...
    fn open_file(&self, path: &str) -> vfs::VfsResult<Box<dyn vfs::SeekAndRead + Send>> {
        ...
        let primed = self
            .source
            .prime_volume(&file_entry.volume_filename, data_start..data_end)?;
...
mod async_impl {
        ...
        T: AsyncPrime + Debug + Send + Sync + 'static,
    {
        ...
        async fn open_file(
            &self,
            path: &str,
        ) -> vfs::VfsResult<Box<dyn SeekAndRead + Send + Unpin>> {
            let entry = self.entry_at(path)?;
            let VfsEntryMeta::File(file_entry) = entry else {
                return Err(VfsError::from(VfsErrorKind::Other("not a file".into())));
            };
        ...
        async fn create_file(
            &self,
            _path: &str,
        ) -> vfs::VfsResult<Box<dyn async_std::io::Write + Send + Unpin>> {
            Err(VfsErrorKind::NotSupported.into())
        }
